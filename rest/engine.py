...
class WhisperEngine:
    ...
    async def transcribe_stream_pcm(
        self,
        audio_pcm: np.ndarray,
        language: str | None = None,
        prompt: str | None = None,
    ):
        """Streaming transcription from raw PCM f32 numpy array.

        Unlike transcribe_stream which collects all segments before yielding,
        this method uses an asyncio.Queue bridge to yield segments as the
        synchronous faster-whisper iterator produces them.

        Args:
            audio_pcm: PCM f32 numpy array (16kHz mono).
            language: Optional BCP-47 language code.
            prompt: Optional initial prompt to guide transcription.

        Yields:
            SegmentResult for each segment, then a final TranscriptionResult.
        ...
        queue_wait_ms = (time.perf_counter() - queue_start) * 1000
        segment_queue: asyncio.Queue[SegmentResult | None] = asyncio.Queue()
        error_holder: list[Exception] = []

        def _inference_worker() -> tuple[str, float]:
            """Run inference in thread pool, push segments to queue."""
            if self._model is None:
                raise TranscriptionError("Whisper model is not loaded")
            try:
                segments_iter, info = self._model.transcribe(
        ...
        try:
            ...
            detected_language, duration = await task

            if error_holder:
                raise TranscriptionError(
                    f"Transcription failed: {error_holder[0]}"
                ) from error_holder[0]
            ...
            logger.debug(
                "PCM stream complete: lang=%s dur=%.2fs queue=%.1fms",
                detected_language,
                duration,
                queue_wait_ms,
            )
